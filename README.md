...
```text
Discord /review · Telegram bot · YouTube video URL · manual paste
      ↓
...
YouTube imports are deduplicated by comment id, so re-importing a video never creates duplicate incidents. Imports run on a short time budget and continue where they left off on the next call.
