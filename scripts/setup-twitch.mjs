 ...
 *
 * Client id/secret: https://dev.twitch.tv/console/apps (create an app,
 * set the OAuth redirect to http://localhost:3000).
 */
...
async function userToken() {
  ...
  console.log(
    "\n2. After authorizing you'll be redirected to localhost:3000 with a code in the address bar.",
  );
  const rl = createInterface({ input: process.stdin, output: process.stdout });
  const code = (await rl.question("3. Paste the code from the redirect URL: ")).trim();
  rl.close();
  const res = await fetch("https://id.twitch.tv/oauth2/token", {
    method: "POST",
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
